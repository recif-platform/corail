...
class MemoryManager:
    ...
    async def extract_from_conversation(self, messages: list[dict]) -> None:
        ...
        try:
            ...
            # Background task: log a single line and move on instead of
            # dumping a traceback on every conversation turn. The chat
            # response is already delivered at this point.
            logger.warning("memory extraction skipped: %s: %s", type(exc).__name__, exc)
