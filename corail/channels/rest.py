...
def _log_chat_trace(user_input: str, conversation_id: str, history_length: int, clean_output: str, raw_output: str, collected_events: list[dict] | None = None) -> str | None:
        ...
        for evt in collected:
            ...
            if etype == "llm_call_completed":
                ...
                with mlflow.start_span(name="guard_blocked", span_type="CHAIN"):
                    mlflow.get_current_active_span().set_attributes({"direction": evt.get("direction", ""), "reason": evt.get("reason", "")})
...
class RestChannel(Channel):
    ...
    async def _chat(self, request: ChatRequest) -> JSONResponse:
        ...
        output = await self._traced_execute(
            cid, request.input, history, request.options,
        )
    ...
    async def _traced_execute(
        self, cid: str, user_input: str,
        history: list[dict[str, str]], options: dict,
    ) -> str:
                ...
                if _HAS_MLFLOW and full_response:
                    ...
                    try:
                        tid = await asyncio.wait_for(
                            asyncio.to_thread(_log_chat_trace, user_input, cid, len(history), _clean, full_response, _events),
                            timeout=5.0,
                        )
                        if tid:
