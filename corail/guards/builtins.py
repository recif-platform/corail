...
class PIIGuard(Guard):
    ...
    async def check(self, content: str, direction: GuardDirection) -> GuardResult:
        ...
        if found:
            if self._block:
                return GuardResult(allowed=False, reason=f"PII detected: {', '.join(found.keys())}", details={"pii_types": list(found.keys())})
            return GuardResult(allowed=True, sanitized=sanitized if self._mask else "", details={"pii_types": list(found.keys()), "masked": self._mask})
