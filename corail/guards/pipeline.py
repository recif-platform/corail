...
class GuardPipeline:
    ...
    async def _run(self, content: str, direction: GuardDirection, user_id: str, session_id: str) -> GuardResult:
        ...
        for guard in self._guards:
            ...
            # Emit event
            event_type = EventType.GUARD_BLOCKED if not result.allowed else (
                EventType.GUARD_INPUT_CHECKED if direction == GuardDirection.INPUT else EventType.GUARD_OUTPUT_CHECKED
            )
            if self._event_bus:
                await self._event_bus.emit(Event(
                    ...
                    data={"guard": guard.name, "direction": direction.value, "allowed": result.allowed, "reason": result.reason},
                ))
