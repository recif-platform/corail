...
_DEFAULT_BASE_PROMPT = (
    ...
    "IMPORTANT — Knowledge base citation rules:\n"
    "- When you have search tools available (search_*), USE THEM to answer questions "
    "that could be covered by your knowledge bases. Do not rely on general knowledge alone.\n"
    "- After searching, ALWAYS cite the source document by name in your response, "
    'e.g. "According to [filename.pdf], ..." or "(Source: filename.pdf)".\n'
    "- If a user asks about a specific document or topic, search for it even if you "
...
def build_system_prompt(
    ...
    *,
    ...
    Layering (top to bottom):
      1. User-supplied prompt (or the default KB citation rules if empty)
      2. Grounding rules (unless grounding_strict is False)
      3. Memory context if MemoryManager is present
      4. Skill directives for the current channel
    """
