...
class UnifiedAgentStrategy(AgentStrategy):
    ...
    def _end_llm_span(self, response: ModelResponse, messages: list[dict] | None = None) -> None:
        ...
        if messages:
            inputs["messages"] = [
                {"role": m.get("role", ""), "content": str(m.get("content", ""))[:200]} for m in messages[-4:]
            ]
        ...
        if response.tool_calls:
            outputs["tool_calls"] = [{"name": tc.name, "args": tc.args} for tc in response.tool_calls]
    ...
    async def _execute_tool_with_retry_full(self, tool_call: ToolCall) -> ToolResult:
        ...
        for attempt in range(_MAX_TOOL_RETRIES + 1):
            ...
            logger.warning(
                "Tool %s failed (attempt %d/%d): %s", tool_call.name, attempt + 1, _MAX_TOOL_RETRIES + 1, last_error
            )
