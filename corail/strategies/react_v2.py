...
class ReActV2Strategy(AgentStrategy):
    ...
    async def _execute_native(
        ...
        for round_num in range(self._budget.max_rounds):
            ...
            await self._events.emit(
                EventType.LLM_CALL_COMPLETED, round=round_num, stop_reason=response.stop_reason
            )
    ...
    async def _stream_native(
        ...
        for round_num in range(self._budget.max_rounds):
            ...
            for tc in response.tool_calls:
                ...
                yield ToolEndEvent(
                    tool=tc.name, output=result_text, success=result.success, call_id=tc.id,
                )
    ...
    async def _execute_tool_with_retry_full(self, tool_call: ToolCall) -> ToolResult:
        ...
        for attempt in range(_MAX_TOOL_RETRIES + 1):
            ...
            if result.success:
                await self._events.emit(
                    EventType.TOOL_RESULT, name=tool_call.name, output=result.output
                )
                return result
        ...
        return ToolResult(
            success=False, output="", error=f"After {_MAX_TOOL_RETRIES + 1} attempts: {last_error}",
        )
