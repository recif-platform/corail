    ...
    for kb in kb_configs:
        try:
            ...
            description = kb.get("description") or (
                f"Search the '{kb_name}' knowledge base for relevant documents. "
                f"PREFER this over web_search when the question relates to {kb_name}. "
                f"Returns document excerpts with source citations."
            )
            tool = KBSearchTool(
                name=tool_name,
