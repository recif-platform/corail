...
def background_model(fallback: Model) -> Model:
    ...
    except Exception as exc:  # noqa: BLE001 — any resolution failure should not break chat
        logger.warning(
            "CORAIL_BACKGROUND_MODEL=%r could not be resolved (%s), "
            "falling back to main chat model",
            uri, exc,
        )
        return fallback
